<?php 
    ...
    $R_Capacity = $_POST['R_Capacity'];
        ...
        $sql = "INSERT INTO room (R_Image,R_Name,R_Description,R_Capacity)
        VALUES ($R_Image, $R_Name, $R_Description, $R_Capacity)";
        if ($conn->query($sql) === TRUE) {
        echo "<script type='text/javascript'>alert('เพิ่มข้อมูลห้องเรียบร้อยเเล้ว');
        window.location = '../../Room_Admin.php';
        </script>";
        } else {
        echo "<script type='text/javascript'>alert('ไม่สามารถเพิ่มข้อมูลห้องได้');
        window.location = '../../Room_Admin.php';
        </script>";
        }
