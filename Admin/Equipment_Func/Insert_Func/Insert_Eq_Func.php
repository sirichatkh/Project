<?php 
    ...
    $Eq_Amount = $_POST['Eq_Amount'];

        $sql = "INSERT INTO equipment (Eq_Image, Eq_Name, Eq_Amount)
        VALUES ($Eq_Image, $Eq_Name, $Eq_Amount)";
        if ($conn->query($sql) === TRUE) {
        echo "<script type='text/javascript'>alert('เพิ่มข้อมูลอุปกรณ์เรียบร้อยเเล้ว');
        window.location = '../../Equipment_Admin.php';
        </script>";
        } else {
        echo "<script type='text/javascript'>alert('ไม่สามารถเพิ่มข้อมูลอุปกรณ์ได้');
        window.location = '../../Equipment_Admin.php';
        </script>";
        }
