<?php 
    ...
    $S_Email = $_POST['S_Email'];
        ...
        $sql = "INSERT INTO staff (S_Image, S_Name, S_Position, S_Skill, S_Email)
        VALUES ($S_Image, $S_Name, $S_Position, $S_Skill, $S_Email)";
        if ($conn->query($sql) === TRUE) {
        echo "<script type='text/javascript'>alert('เพิ่มข้อมูลบุคลากรเรียบร้อยเเล้ว');
        window.location = '../../Staff_Admin.php';
        </script>";
        } else {
        echo "<script type='text/javascript'>alert('ไม่สามารถเพิ่มข้อมูลบุคลากรได้');
        window.location = '../../Staff_Admin.php';
        </script>";
        }
